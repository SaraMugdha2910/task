...
<body class="bg-gray-100 min-h-screen flex flex-col items-center">
    ...
    <div class="w-[80%] space-y-6">
      ...
      <div class="bg-white shadow-md rounded-md p-6 flex flex-col items-center w-full">
        ...
        <form id="excel-upload-form" enctype="multipart/form-data" class="w-full space-y-4">
          @csrf
          <label
            class="flex flex-col items-center justify-center w-full h-24 border-2 border-dashed border-gray-300 rounded-md cursor-pointer hover:border-blue-400 hover:bg-blue-50 transition">
      ...
      <!-- Preview Table Box -->
      <div id="preview-table" class="bg-white shadow-md rounded-md  hidden w-full"></div>
 ...
 function loadPage(url) {
    ...
    const contractorId = $('#subcontractor-table').data('contractor-id'); 
console.log(contractorId);
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            feather.replace();
            attachPagination(); // reattach click handlers
        },
        complete: function() {
            $('#spinner-overlay').addClass('hidden');
        }
...
function attachPagination() {
    // Laravel generates ul > li > a
    $('#preview-table').find('ul.pagination a').off('click').on('click', function(e) {
        e.preventDefault();
